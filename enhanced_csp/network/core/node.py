...
from .types import NodeID, NodeCapabilities, PeerInfo, NetworkMessage, MessageType
        ...
        try:
            ...
            if self.config.enable_adaptive_routing and self.routing:
                self.adaptive_routing = AdaptiveRoutingEngine(
                    self, self.config.routing, self.routing
                )
        ...
        while self.is_running:
            try:
                ...
                if hasattr(self.routing, "routing_table"):
                    self.metrics["routing_table_size"] = len(
                        getattr(self.routing, "routing_table", {})
                    )
        ...
        while self.is_running:
            try:
                ...
                if self.stats["start_time"]:
                    ...
                    logger.info(
                        f"Node stats - Uptime: {uptime:.0f}s, "
...
class EnhancedCSPNetwork:
    ...
    def __init__(self, config: Optional[NetworkConfig] = None):
        ...
        # DNS overlay stub (initialized when starting if enabled)
        self.dns_overlay = None
        ...
        try:
            ...
            if self.config.enable_dns:
                try:
                    ...
                    self.dns_overlay = DNSOverlay(default_node)
                    await self.dns_overlay.start()
                except ImportError:
                    logger.warning("DNS overlay not available")
                    self.dns_overlay = type("DNSStub", (), {"records": {}})()

            # Update metrics
            self._metrics["nodes_active"] = len(self.nodes)
    ...
    def get_peers(self) -> List[PeerInfo]:
        """Get all peers from all nodes."""
        all_peers = []
        for node in self.nodes.values():
            all_peers.extend(node.peers.values())
        return all_peers

    async def metrics(self) -> Dict[str, Any]:
        """Get network metrics as a method."""
        # Update uptime
        ...
        for name, node in self.nodes.items():
            if hasattr(node, "stats"):
                total_messages_sent += node.stats.get("messages_sent", 0)
                total_messages_received += node.stats.get("messages_received", 0)
