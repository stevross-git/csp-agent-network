 ...
 */
import { defaultRolesMock } from "@/utils/apiFallbackData";
...
class RoleManager {
  ...
  renderRows() {
    ...
    this.tbody.innerHTML = this.roles
      .map(
        (role) => `
  ...
  async addRole(data) {
    const res = await this.apiRequest("/api/roles", {
      method: "POST",
  ...
  async updateRole(roleId, data) {
    const res = await this.apiRequest(`/api/roles/${roleId}`, {
      method: "PUT",
      body: JSON.stringify(data),
    });
  ...
  openEditRoleModal(role) {
    if (
      ...
      window.adminModalManager.openEditRoleModal
    ) {
