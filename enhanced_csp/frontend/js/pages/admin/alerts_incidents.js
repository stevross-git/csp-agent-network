...
class AlertsIncidentsManager {
  ...
  async init() {
    ...
    try {
      ...
      console.error(
        "❌ Failed to initialize Alerts & Incidents Manager:",
        error,
      );
  ...
  async loadAlerts() {
    try {
      ...
      const response = await fetch(
        `${this.endpoints.alertmanager}/api/v1/alerts`,
      );
      if (response.ok) {
  ...
  processMetrics(metrics) {
    ...
    if (metrics.up && metrics.up.result) {
      const upServices = metrics.up.result.filter(
        (m) => m.value[1] === "1",
      ).length;
  ...
  showFallbackData() {
    ...
    this.showNotification(
      "Using fallback data - external monitoring services unavailable",
      "warning",
    );
  }
  ...
  generateFallbackAlerts() {
    return fallbackAlertsMock;
  }

  generateFallbackIncidents() {
    return fallbackIncidentsMock;
  }
  ...
  renderIncidents() {
    ...
    container.innerHTML = this.incidents
      .map(
        (incident) => `
  ...
  updateSummaryCards() {
    ...
    const infoCount = this.alerts.filter(
      (a) => a.severity === "info" && a.status === "firing",
    ).length;
    const incidentsCount = this.incidents.filter(
      (i) => i.status !== "resolved",
    ).length;
  ...
  acknowledgeAll() {
    ...
    this.showNotification(
      `${firingAlerts.length} alerts acknowledged`,
      "success",
    );
  }
