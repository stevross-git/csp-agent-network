...
import { settingsMock } from "@/utils/apiFallbackData";
/**
...
class SystemManager {
  ...
  bindEvents() {
    ...
    document.addEventListener("change", (e) => {
      if (e.target.classList.contains("critical-setting")) {
        this.showToast(
          "Critical setting changed - remember to save!",
          "warning",
        );
      }
    });
  ...
  renderFilterBar() {
    ...
    const categories = [
      ...new Set(this.settings.map((s) => s.category || "General")),
    ].sort();
  ...
  filterSettings(searchTerm, categoryFilter) {
    const categories =
      this.formContainer.querySelectorAll(".settings-category");
  ...
  toggleAllCategories(expand) {
    const categoryBodies =
      this.formContainer.querySelectorAll(".category-body");
    const toggleIcons =
      this.formContainer.querySelectorAll(".category-toggle i");

    categoryBodies.forEach((body, index) => {
      body.style.display = expand ? "block" : "none";
      if (toggleIcons[index]) {
        toggleIcons[index].className = expand
          ? "fas fa-chevron-down"
  ...
  async resetSettings() {
    try {
      ...
      if (
        !confirm(
          "Are you sure you want to reset all settings to their original values? This will discard any unsaved changes.",
        )
      ) {
        return;
      }
  ...
  setLoading(loading) {
    ...
    const existingOverlay =
      this.formContainer?.querySelector(".loading-overlay");
