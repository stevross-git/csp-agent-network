...
import {
  apiFallbackData,
  infrastructureMetricsMock,
  infrastructureServicesMock,
...
class InfrastructureManager {
  ...
  initializeWebSocket() {
    try {
      const wsUrl =
        this.apiBaseUrl.replace("http", "ws") + "/ws/infrastructure";
    ...
    while (retries < this.config.maxRetries) {
      try {
        ...
        if (!response.ok) {
          ...
          if (response.status === 404 && apiFallbackData[key]) {
            console.warn(
              `Endpoint not found: ${endpoint}. Using fallback data.`,
            );
            return apiFallbackData[key];
          }
  ...
  initializeDefaultMetrics() {
    const defaultMetrics = infrastructureMetricsMock;
    this.updateMetrics(defaultMetrics);
  }

  initializeDefaultServices() {
    const defaultServices = infrastructureServicesMock;
  ...
  startMonitoring() {
    ...
    console.log(
      `📊 Started real-time monitoring (${this.config.refreshRate}ms interval)`,
    );
  }
  ...
  getMetricHistory(metric) {
    ...
    return infrastructureMetricHistory;
  }
  ...
  async restoreBackup(backupId) {
    ...
    try {
      ...
      const result = await this.apiRequest(
        `/api/infrastructure/backup/${backupId}/restore`,
        {
          method: "POST",
        },
      );
  ...
  async restartService(serviceName) {
    ...
    try {
      ...
      await this.apiRequest(
        `/api/infrastructure/services/${serviceName}/restart`,
        {
          method: "POST",
        },
      );
  ...
  async stopService(serviceName) {
    ...
    try {
      await this.apiRequest(
        `/api/infrastructure/services/${serviceName}/stop`,
        {
          method: "POST",
        },
      );
      ...
      this.showNotification(
        `Failed to stop ${serviceName}: ${error.message}`,
        "error",
      );
    }
  }
  ...
  async startService(serviceName) {
    try {
      ...
      await this.apiRequest(
        `/api/infrastructure/services/${serviceName}/start`,
        {
          method: "POST",
        },
      );
  ...
  renderMetrics() {
    const metricsHtml = Array.from(this.metrics.entries())
      .map(
        ([name, data]) => `
  ...
  renderServices() {
    const servicesHtml = Array.from(this.services.entries())
      .map(
        ([name, service]) => `
  ...
  attachEvents() {
    // Main action buttons
    this.section
      .querySelector("#infra-refresh-btn")
      ?.addEventListener("click", () => this.refresh());
    this.section
      .querySelector("#infra-create-backup-btn")
      ?.addEventListener("click", () => this.createBackup());
    this.section
      .querySelector("#infra-settings-btn")
      ?.addEventListener("click", () => this.showSettings());
    this.section
      .querySelector("#infra-fullscreen-btn")
      ?.addEventListener("click", () => this.toggleFullscreen());
    this.section
      .querySelector("#infra-export-logs-btn")
      ?.addEventListener("click", () => this.exportLogs());
    this.section
      .querySelector("#infra-maintenance-btn")
      ?.addEventListener("click", () => this.toggleMaintenanceMode());
    this.section
      .querySelector("#infra-emergency-shutdown-btn")
      ?.addEventListener("click", () => this.emergencyShutdown());
  ...
  async toggleMaintenanceMode() {
    ...
    try {
      ...
      this.showNotification(
        `Failed to ${action} maintenance mode: ${error.message}`,
        "error",
      );
    }
  }
  ...
  async emergencyShutdown() {
    const confirmation = prompt(
      'Type "EMERGENCY SHUTDOWN" to confirm this action:',
    );
    ...
    try {
      ...
      this.showNotification(
        "Failed to initiate emergency shutdown: " + error.message,
        "error",
      );
    }
  }
