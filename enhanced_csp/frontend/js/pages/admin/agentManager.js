 ...
 */
import { managerSampleAgents } from "@/utils/apiFallbackData";
...
export class AgentManager {
  ...
  async loadAgents() {
    try {
      ...
      const sampleAgents = managerSampleAgents;
  ...
  renderAgents() {
    ...
    agentGrid.innerHTML = filteredAgents
      .map((agent) => this.createAgentCard(agent))
      .join("");
  }
  ...
  async deployAllAgents() {
    try {
      ...
      const deployBtn = document.querySelector(
        'button[onclick="deployAllAgents()"]',
      );
      if (deployBtn) {
      ...
      this.adminPage.showSuccess(
        "Deploy Complete",
        `Deployed ${inactiveAgents.length} agents`,
      );
    } catch (error) {
  ...
  updateRealTimeData() {
    ...
    this.agents.forEach((agent) => {
      if (agent.status === "active") {
        ...
        agent.cpuUsage = Math.max(
          1,
          Math.min(100, agent.cpuUsage + cpuVariation),
        );
        agent.memoryUsage = Math.max(
          1,
          Math.min(100, agent.memoryUsage + memVariation),
        );
        agent.lastActivity = new Date();
      }
    });
